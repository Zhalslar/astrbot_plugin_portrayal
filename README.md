...
3. 仍然失败时，说明浏览器没能访问到插件接口（面板接口路径为
   `/api/v1/plugins/extensions/astrbot_plugin_portrayal/*`）。可在浏览器开发者工具的
   Network 里确认该请求的状态码：401/403 是登录态问题（重新登录面板），404 说明插件未加载成功
   （检查插件是否启用、启动日志有无报错）。
